...
const characterService = {
  ...
  async getOrCreateCharacter(filename) {
    ...
    try {
      ...
      throw new CharacterNotFoundError(
        `Character file ${filename} not found or invalid`
      );
    }
  },
  ...
  async dealDamage(filename, amount, type) {
    ...
    if (!type) {
      throw new ValidationError("Damage type must be provided");
    } else if (typeof type !== "string") {
      throw new ValidationError("Damage type must be a string");
    }
